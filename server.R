...
players_names_col <- seq(from=which(colnames(scores)=='Annonces') + 1, to= ncol(scores))
...
shinyServer(function(input, output, session) {
  ...
  ##onglet scores ----
  observeEvent(input$active_players,{
    if (length(input$active_players) == 5){
      updateButton(session, "valid", disabled = FALSE)

    }else{
      updateButton(session, "valid", disabled = TRUE)
    }
  })
  
  shinyjs::onclick('scores',{
    if (!is.null(input$scores_rows_selected)){
      updateButton(session, "modify", disabled = FALSE)
      updateButton(session, "delete", disabled = FALSE)
    }else{
      updateButton(session, "modify", disabled = TRUE)
      updateButton(session, "delete", disabled = TRUE)
    }
  })
  ...
  observeEvent(input$valid, {
    ...
    if (row==1){
      scores[row,players_names_col] <- 0
    }else{
      scores[row,players_names_col] <- scores[row - 1, players_names_col]
    }
    ...
    observeEvent(input$modify,{
      showModal(
        modalDialog(
          tagList(
            print(input$scores_rows_selected)
          ),
          title="Modifier",
          footer = tagList(actionButton("confirmModify", "Oui"),
                           modalButton("Non")
          )
        )
      )
              ...
              modalDialog(
                tagList(
                  paste('Est-vous sûre de vouloir supprimer la ligne', 
                        input$scores_rows_selected)
                ),
                title="Suppression",
                footer = tagList(actionButton("confirmDelete", "Oui"),
                                 modalButton("Non")
                                 )
                )
              )
    ...
    observeEvent(input$confirmDelete,{
      ...
      scores <- read.csv("www/scores.csv", sep = ';', stringsAsFactors=FALSE, dec=',')
      scores <- scores[-input$scores_rows_selected,]
      rownames(scores)[as.numeric(rownames(scores))>input$scores_rows_selected] <-
        as.numeric(rownames(scores)[as.numeric(rownames(scores))>input$scores_rows_selected]) - 1
